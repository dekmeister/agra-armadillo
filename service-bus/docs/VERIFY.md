...
| P2 | Their message-cost / robustness characterisation | Guide §5 table; `docs/03` §4 L56–71; **L3's `principle` string** | Especially Bully ~O(n²) and Raft's quorum stall, which the game teaches as fact. **Raised in prominence by WP6.4:** Phase 3's Objective card now states "choosing an election method is choosing a message cost and a failure mode" for the whole level. Phrased as the trade the game *models* rather than as a sourced A-GRA claim, but it deserves a real source. |
...
| P5 | Team split keeps the original PackageID; leaderless half re-forms and re-elects; **never auto-merges** | `docs/01` L56; L7 `mergeTeam`; **L7's `principle` string** | The "never auto-merge" rule is load-bearing for Phase 7. **Raised in prominence by WP6.4:** it is now half of the sentence permanently on Phase 7's Objective card ("split halves merge only on command"), so it is asserted to the player continuously rather than once in a beat. |
