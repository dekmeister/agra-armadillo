...
| Game element | A-GRA concept | Fidelity notes |
|---|---|---|
| **Role badge** on each C2 node | **RBAC roles**: Admin, QB, AVC, LRE, Observer | **Unverified (VERIFY C1, C2).** Admin can re-permission others; QB has full command but not re-permissioning; AVC = flight tasks, weapon-restricted by default; LRE = takeoff/landing; Observer = read-only. The XSD cannot settle this: it models an operator role as *configured data* (`MA_OperatorRoleMDT.Role` is a `ForeignKeyType` with a free-text `RoleDescription`), referenced by whatever gates on it — never as an enumeration. Only the absent C2 Volume can say whether A-GRA names a standard set. |
| **A delivered-but-rejected message** (turns red at MA) | MA checks authorisation; unauthorised → command ignored, `...StatusMT` = `REJECTED` (`CannotComply`) | Faithful and a core mechanic: *arrival ≠ effect*. Both reply values are sourced: `ApprovalStatusEnum` = APPROVED/REJECTED/PENDING/CANCELED, and `CannotComplyEnum.INELIGIBLE_CONTROL_SOURCE` is annotated as rejection because the source "hasn't been granted permission to control" the System or Capability. |
| **Weapon-release "approval gate"** that only a QB can satisfy | **Target Authority** + `MA_ApprovalRequestMT/StatusMT`; or `MA_DesignationRequestMT→MA_DesignationMT` | Faithful, and now sourced (VERIFY C4 closed): `MA_RulesOfEngagementMDT.TargetAuthorityCriteria` "specifies who is authorized to approve or set something as a Target", and describes the designation route as a `MA_DesignationRequestMT` "sent to be approved by the related Target Authority who would then send a `MA_DesignationMT`". `MA_ApprovalRequestMT` carries an `Approver` role and a `RespondBy` deadline. `[S]` The full ROE machinery (Identity Matrix, WEZ, Target Custody, geozones) is collapsed to a single gate flag, and the `StrikeConsentRequestMT` round trip required when `StrikeConsentRequired` is TRUE is omitted entirely. `[S]` A-GRA falls back to the approval policy's default response when `RespondBy` lapses; the game just lets the window close. |
...
| Game element | A-GRA concept | Fidelity notes |
|---|---|---|
| **Pre-mission "election policy" dial** | The four named methods, enumerated 0–3 on `MA_LeadershipMetricsMDT.PackageLeaderElectionMethod` (XSD): **Bully, Static Fitness Score, Maximum Consensus, Raft** | Faithful list, and the *dial* is faithful too — the schema treats the method as package configuration with an implementor default. Each has a real, distinct message pattern & cost (see `03-failure-degradation.md`). Earlier revisions listed a fifth, "Off-Nominal"; it has no source and was removed. |
| **Leadership fitness score** on each ACP | `MA_PartnerMetricsType` carries `StaticLeadershipFitnessScore` and `DynamicLeadershipFitnessScore`; dynamic determination is one of `C2_CommsBinary` or `P2P_CommsCount` (XSD `MA_DynamicFitnessScoreChoiceType`) | Faithful — dynamic fitness really does key off comms health, by one of two named methods. `[S]` A-GRA states **no** tiebreak; the sim breaks ties by node id for determinism. (An earlier "highest tail number" claim was unsourced and is gone.) |
