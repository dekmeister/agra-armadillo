...
| Producer publish cadence is a player control; a `-1` datum goes stale after a fixed `staleAfter` window (W0's 0-2) | Status-1/Data-1 are "asynchronous and/or periodic" (SPC-001 §5.1.6) — the standard leaves periodicity to the producer and prescribes no schedule and no freshness/staleness rule | Publish timing/cadence is a game control and `staleAfter` is a pure game construct (no spec analogue); disclosed here and in the sheet's Fidelity Notes. What is real: republication is the only recourse under `-1` (no ack), which the staleness window forces the player to internalize |
