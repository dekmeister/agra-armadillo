...
normal-form/
  packages/
    ...
    levels/      # data only
      worlds/*/sheet-*.json  # goal, palette, seeds, fidelity notes, citations
    ...
    game/        # React app
      ...
      src/meta/              # level select, how-to-play, UCI reference codex,
                             # debrief, fidelity notes panel
  tools/
...
interpreter lives in `core`. Benefits: export/share, replay is structural, and
the editor is just a view.

**Reference machines prove solvability.** Each sheet ships (test-only, not
bundled) a reference machine; golden tests prove every sheet solvable on all its
seeds — the Brain Swap pattern (`packages/core` reference brains). A sheet
without a passing reference machine cannot ship. (Pars/scoring were cut at the
MVP review; the reference machine's only job now is the solvability proof.)
...
| Forge (W3, post-1.0) type model scope creep | the bench models only what the cited CERTs police (compositor, base type, naming, ID typing, version attribute); it is not an XSD editor |
