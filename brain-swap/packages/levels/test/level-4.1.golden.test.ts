...
import { level41, level41NaiveBrain, level41ReferenceBrain, scenarioFor } from "@brain-swap/levels";

// Level 4.1 "Second Body": the 1.2 HSA mission re-flown on the Heron (AX-02). A
// profile-reading brain ports unchanged; a brain that hardcoded the Mule's Speed:60
// is rejected by the Heron's slower envelope.

const MAX_STEPS = 500;

function solve(brain = level41ReferenceBrain): World {
  return run(initWorld(scenarioFor(level41, brain)), MAX_STEPS);
}
...
const GOLDEN_LOG = [
  ...
  "t28 FA->MA MA_FlightActivityMT [delivered]",
  ...
  "t32 FA->MA MA_PositionReportDetailedMT [delivered]",
  "t33 FA->MA MA_PositionReportDetailedMT [delivered]",
  "t33 FA->MA MA_FlightActivityMT [delivered]",
];

describe("level 4.1 golden run (portable reference brain)", () => {
  it("the profile-reading brain ports to the Heron and wins", () => {
    const w = solve();
    expect(w.outcome).toBe("won");
    expect(w.holdTicks).toBeGreaterThanOrEqual(level41.objective.holdTicks);
  });
  ...
  it("reads cruise/loiter speed from the Heron's profile (MaxAirspeed 50 / MinAirspeed 25)", () => {
    const sends = solve()
  ...
  it("produces stable scores equal to par", () => {
    expect(scoreWorld(solve())).toEqual(level41.pars);
  });
...
describe("level 4.1 negative run (brain that hardcoded the Mule's Speed)", () => {
  it("Speed:60 exceeds the Heron's envelope and is rejected; the level is never won", () => {
    const w = solve(level41NaiveBrain);
