...
// source) solves it in a deterministic, byte-stable golden run, so the auto-play
// the player watches is provably solvable. The log is identical to 1.2's modulo
// the level id.

const MAX_STEPS = 500;

function solve(): World {
  return run(initWorld(scenarioFor(level00, level00ReferenceBrain)), MAX_STEPS);
}
...
describe("level 0.0 golden run (reference brain)", () => {
  it("the reference brain wins the level", () => {
    const w = solve();
    expect(w.outcome).toBe("won");
    expect(w.holdTicks).toBeGreaterThanOrEqual(level00.objective.holdTicks);
  });
