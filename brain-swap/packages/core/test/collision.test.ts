  ...
  capabilities: [
    {
      id: "FX-1",
      type: "HSA_CSA",
      profile: { minAltitude: 0, maxAltitude: 9000, minAirspeed: 20, maxAirspeed: 120 },
    },
  ],
...
const cmd = (state: "NEW" | "UPDATE", heading: number) =>
  msg("MA_FlightCommandMT", "MA", "FA", {
    ...
    CapabilityID: "FX-1",
    Heading: heading,
    ...(state === "NEW" ? { Altitude: 3000, Speed: 50 } : {}),
  });
...
const lastCmdStatus = (w: World) =>
  w.log.filter((e) => e.type === "MA_FlightCommandStatusMT").at(-1);
...
function flyIntoThreat(): World {
  ...
  w = injectMA(
    w,
    msg("MA_ControlRequestMT", "MA", "FA", { RequestType: "ACQUIRE", CapabilityID: "FX-1" }),
  );
...
describe("collision interrupt", () => {
  ...
  it("rejects a re-issued dangerous vector with VIOLATION_AIR_TRAFFIC (hold persists)", () => {
    ...
    w = stepUntil(
      w,
      (x) =>
        (lastCmdStatus(x)?.payload as { CommandProcessingState?: string })
          ?.CommandProcessingState === "REJECTED",
    );
    const rej = lastCmdStatus(w)!;
    expect((rej.payload as { CommandProcessingState?: string }).CommandProcessingState).toBe(
      "REJECTED",
    );
    expect((rej.payload as { ValidationResult?: string }).ValidationResult).toBe(
  ...
  it("accepts a clear vector and hands control back (releases the hold)", () => {
    ...
    w = stepUntil(
      w,
      (x) =>
        (lastCmdStatus(x)?.payload as { CommandID?: string })?.CommandID === "CMD-1" &&
        (lastCmdStatus(x)?.payload as { CommandProcessingState?: string })
          ?.CommandProcessingState === "ACCEPTED",
    );
    expect(
      (lastCmdStatus(w)!.payload as { CommandProcessingState?: string }).CommandProcessingState,
    ).toBe("ACCEPTED");
  });
